...
export async function PUT(
  ...
  try {
    ...
    if (!userId) {
      console.log("there is not a userId");
      return Response.json({ error: "Unauthorized" }, { status: 401 });
    }

    console.log("checking to see if user has rights to edit note");
    let userIsAdmin = await isAdmin(userId);

    // if (userId !== note.userId || !userIsAdmin) {
    //   return Response.json({ error: "Unauthorized" }, { status: 401 });
    // }
    console.log("user has rights to modify note");
    const embedding = await getEmbeddingForNote(title, content);
...
export async function DELETE(
  ...
  try {
    console.log("inside of DELETE /api/owner/notes/[companionId]");
    const body = await req.json();
    const companion = await prisma.companion.findUnique({
      where: {
        id: params.companionId,
